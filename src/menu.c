...
static struct
{
	...
	//Menu assets
	Gfx_Tex tex_back, tex_ng, tex_story, tex_title, tex_title1;
	FontData font_bold;
...
void Menu_Load(MenuPage page)
{
	...
	Gfx_LoadTex(&menu.tex_title1, Archive_Find(menu_arc, "title1.tim"), 0);
	Mem_Free(menu_arc);
	
	FontData_Load(&menu.font_bold, Font_Bold);
	
	menu.gf = Char_GF_New(FIXED_DEC(300,1), FIXED_DEC(300,1));
...
void Menu_Tick(void)
{
	...
	//Get song position
	u16 next_step = Audio_TellXA_Milli() / 105; //100 BPM
		...
		case MenuPage_Opening:
		{
			...
			else
			{
				...
				switch (beat)
				{
					...
					case 5:
					case 4:
						menu.font_bold.draw(&menu.font_bold, "I FUCKING LOVE", SCREEN_WIDTH2, SCREEN_HEIGHT2, FontAlign_Center);
						menu.font_bold.draw(&menu.font_bold, "CRASH BANDICOOT", SCREEN_WIDTH2, SCREEN_HEIGHT2 + 16, FontAlign_Center);
				//Fallthrough
					case 3:
					case 2:
					case 1:
					case 0:
						...
						menu.font_bold.draw(&menu.font_bold, "BACK AT IT AGAIN", SCREEN_WIDTH2, SCREEN_HEIGHT2 - 16, FontAlign_Center);
						break;
					
					case 13:
					case 12:
					case 11:
						Gfx_BlitTex(&menu.tex_ng, &src_ng, (SCREEN_WIDTH - 190) >> 1, SCREEN_HEIGHT2 - 100);
				//Fallthrough
					case 10:
					case 9:
					case 8:
						menu.font_bold.draw(&menu.font_bold, "CREATED BY", SCREEN_WIDTH2, SCREEN_HEIGHT2 - 64, FontAlign_Center);
						break;
					
					
					case 14:
						menu.font_bold.draw(&menu.font_bold, "WOAH", SCREEN_WIDTH2, SCREEN_HEIGHT2 - 24, FontAlign_Center);
						break;
				}
				break;
			}
	...
	//Fallthrough
		case MenuPage_Title:
		{
			...
			u32 x_rad = (logo_scale * (250 >> 1)) >> FIXED_SHIFT;
			u32 y_rad = (logo_scale * (150 >> 1)) >> FIXED_SHIFT;
			
			RECT logo_src = {0, 0, 254, 180};
			RECT logo_dst = {
				157 - x_rad + (SCREEN_WIDEADD2 >> 1),
				100 - y_rad,
				x_rad << 1,
				y_rad << 1
			};
			Gfx_DrawTex(&menu.tex_title1, &logo_src, &logo_dst);
		...
		case MenuPage_Story:
		{
			static const struct
			{
				...
				{"1", StageId_1_1, "FRIDAY NIGHT WUMPIN", {"WUMP", "CRATES", "SPIN"}},
			};
		...
		case MenuPage_Freeplay:
		{
			...
			//Draw difficulty selector
			Menu_DifficultySelector(SCREEN_WIDTH - 100, SCREEN_HEIGHT2 - 10);
		...
		case MenuPage_Mods:
		{
			static const struct
			{
				...
				{StageId_Kapi_2, "   PORTED CRASH WEEK", false},
				{StageId_Clwn_1, "CUCKYDEV", false},
				{StageId_Clwn_2, "   PSXFUNKIN", false},
				{StageId_Clwn_1, "ILLUZTRIOUZ", false},
				{StageId_Kapi_2, "   CRASH WEEK LEADER", false},
				{StageId_Clwn_1, "BOIDHOUSE", false},
				{StageId_Clwn_1, "  ARTIST AND CRASH VA ", false},
				{StageId_Clwn_1, "MR XBON AND PUNKETT", false},
				{StageId_Clwn_1, "   CHARTS ", false},
				{StageId_Clwn_1, "GAZOZOZ", false},
				{StageId_Clwn_1, "   CODER AND FUNNY NAME ", false},
				{StageId_Clwn_1, "PENGUINSMH", false},
				{StageId_Clwn_1, "   CODER ", false},
				{StageId_Clwn_1, "TOMATSUARTS", false},
				{StageId_Clwn_1, "   MENU ARTIST ", false},
				{StageId_Clwn_1, "FLAMBOYANT OBSESSION", false},
